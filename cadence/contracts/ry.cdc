...
pub contract KARAR: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        // Destroy the vault
        destroy() {
            KARAR.totalSupply = KARAR.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            KARAR.totalSupply = KARAR.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&KARAR.Minter>(/public/Minter, target: /storage/MinterStorage)
